...
void WindowFunctionExecutor::Init() {
  ...
  for (const auto &tuple_rid : tuples) {
    ...
    for (unsigned i = 0; i < plan_->columns_.size(); i++) {
      ...
      if (it != plan_->window_functions_.end()) {
        // 该列是窗口函数
        Value value = MakeWindowFunctionValue(&tuple_rid.first, it->second.function_);
        AggregateKey key = MakeWindowFunctionKey(&tuple_rid.first, it->second.partition_by_);
        hash_table_.InsertCombine(key, value, it->second.type_);
        if (having_order_by_) {
          if (it->second.type_ == WindowFunctionType::Rank) {
            ...
            if (last_order_by.CompareEquals(this_order_by) == CmpBool::CmpTrue) {
              ...
              last_rank = hash_table_.GetValueFromMap(key, it->second.type_);
              values.push_back(last_rank);
            }
          } else {
            values.push_back(hash_table_.GetValueFromMap(key, it->second.type_));
          }
        }
      } else {
        if (having_order_by_) {
