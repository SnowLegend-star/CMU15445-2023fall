...
class TableIterator {
 ...
 public:
  ...
  // stop_at_rid：指定一个结束的 RID，当扫描到该记录时，迭代器将停止，防止出现死循环（尤其是在项目 4
  // 中，更新操作可能会涉及删除和插入）。
 ...
 private:
  ...
  // 假设在某些操作中，更新可能涉及删除和插入（例如项目 4
  // 中的操作），如果没有停止条件，迭代器可能会陷入死循环（例如扫描到一个被删除的元组后又插入回表中）。 通过设置
  // stop_at_rid_，可以确保迭代器在达到指定的结束位置后停止。
  RID stop_at_rid_;
};
