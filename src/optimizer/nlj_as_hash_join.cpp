...
void Optimizer::PredParser(const AbstractExpressionRef &predicate,std::vector<AbstractExpressionRef> *left_pred,std::vector<AbstractExpressionRef> *right_pred){
  // 拆分逻辑表达式
...
auto Optimizer::OptimizeNLJAsHashJoin(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  if(optimized_plan->GetType()==PlanType::NestedLoopJoin){  // 针对NLJ进行优化
      ...
      return std::make_shared<HashJoinPlanNode>(optimized_plan->output_schema_,optimized_plan->children_[0],optimized_plan->children_[1],
                                                left_expr,right_expr,nlj_plan.join_type_);
  }
